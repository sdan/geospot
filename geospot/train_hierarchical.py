"""
Hierarchical multi-turn GRPO training for geospot VLM.

Uses HierarchicalGeoEnv: country -> coords (2-turn).
Skips region turn since geomix doesn't have region labels.
"""
...
@chz.chz
class CLIConfig:
    """CLI config for hierarchical RL training."""
    ...
    max_tokens: int = 128  # Per turn
    temperature: float = 1.0

    # Hierarchical config
    turns: list[str] = chz.field(default_factory=lambda: ["country", "coords"])  # Skip region
    teacher_forcing_prob: float = 1.0  # Use ground truth hints

    # Logging
    log_path: str | None = None
    wandb_project: str | None = "geospot-hierarchical"
...
def create_env_from_sample(
    ...
    """Create HierarchicalGeoEnv from a GeoSample."""
    ground_truth = GeoLocation(
        lat=sample.lat,
        lon=sample.lon,
        city=sample.city,
        country=sample.country,
        region=sample.city,  # Use city as region fallback
    )
...
async def run_training(cli: CLIConfig):
    """Main training loop for hierarchical multi-turn."""
    if cli.log_path:
        log_path = cli.log_path
    else:
        model_name = cli.model_name.replace("/", "-")
        timestamp = datetime.now().strftime("%Y-%m-%d-%H-%M")
        log_path = f"/tmp/geospot-hierarchical/{model_name}-{timestamp}"

    check_log_dir(log_path, behavior_if_exists=cli.behavior_if_log_dir_exists)
    os.makedirs(log_path, exist_ok=True)

    logger.info(f"Hierarchical GRPO Training: {cli.hf_repo} -> {log_path}")
    logger.info(f"Model: {cli.model_name}, batch={cli.batch_size}, group={cli.group_size}")
    logger.info(f"Turns: {cli.turns}, teacher_forcing={cli.teacher_forcing_prob}")
    if cli.local_path:
    ...
    if cli.wandb_project:
        wandb.init(
            ...
            name=f"hierarchical-{'-'.join(cli.turns)}",
            tags=["hierarchical", "multi-turn"],
            config={
                ...
                "turns": cli.turns,
                "teacher_forcing_prob": cli.teacher_forcing_prob,
    ...
    db = DBWriter(
        run_id=run_id,
        run_name=f"{cli.hf_repo}-hierarchical",
        run_type="rl-hierarchical",
        config={
            "model_name": cli.model_name,
            "hf_repo": cli.hf_repo,
    ...
    for step in range(cli.max_steps):
        ...
        # Build env groups (group_size copies per sample for GRPO)
        ...
        for sample in samples:
            ...
            for env, (transitions, metrics) in zip(env_group, episode_results):
                ...
                if "distance_km" in metrics:
                    batch_distances.append(metrics["distance_km"])
